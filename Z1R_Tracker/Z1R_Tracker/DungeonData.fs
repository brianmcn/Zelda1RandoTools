...
module Factoids =
    ...
    //        "(how does it work for up-a-ing from a room where you opened that side (via bomb (shutter?))"
        ...
        "Bomb upgrade man never has a bombable north wall, nor do rooms containing dungeon NPCs who give hints. On the other hand, muggers and Hungry Goriya can have bombable north walls."
    ...
    let zTrackerTips = [|
        ...
        "Dr. Brian Lorgon111 made Z-Tracker.  Send him some love on twitch (lorgon) or on YouTube (lorgon111)."
        |]
    let allTips = [|
        yield! noviceTips
