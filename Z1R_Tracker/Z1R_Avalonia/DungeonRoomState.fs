...
let mkTxt(txt) = new TextBox(FontSize=12., Foreground=Brushes.Orange, Background=Brushes.Black, IsReadOnly=true, IsHitTestVisible=false, Padding=Thickness(0.),
...
let scale(bmp, scale) = 
    if bmp = null then
        ...
        RenderOptions.SetBitmapInterpolationMode(icon, Avalonia.Visuals.Media.Imaging.BitmapInterpolationMode.LowQuality)
        icon
...
let dungeonRoomMouseButtonExplainerDecoration =
    ...
    for color, text, pict in [Brushes.DarkMagenta,"Completed room",completed.CurrentDisplay(false); Brushes.DarkCyan,"Uncompleted room",uncompleted.CurrentDisplay(false)] do
        ...
        let tb = new TextBox(FontSize=16., Foreground=Brushes.Orange, Background=Brushes.Black, IsReadOnly=true, IsHitTestVisible=false, Padding=Thickness(0.),
...
let DoModalDungeonRoomSelectAndDecorate(cm:CustomComboBoxes.CanvasManager, originalRoomState:DungeonRoomState, usedTransports:_[], setNewValue, positionAtEntranceRoomIcons) = async {
    ...
    let makeCaption(txt, centered) = 
        let tb = new TextBox(Text=txt, FontSize=16., Foreground=Brushes.Orange, Background=Brushes.Black, BorderThickness=Thickness(0.), IsReadOnly=true, IsHitTestVisible=false, Padding=Thickness(0.),
    ...
    let! allDone = async {
        ...
        let grid = [|
            ...
            |]
        let gridElementsSelectablesAndIDs : (Control*_*_)[] = grid |> Array.mapi (fun _i rt ->
        ...
        let redrawTile(curState:RoomType) =
            ...
            RenderOptions.SetBitmapInterpolationMode(fullRoomDisplay, Avalonia.Visuals.Media.Imaging.BitmapInterpolationMode.LowQuality)
            fullRoomDisplay.Opacity <- tileSunglasses
            canvasAdd(tileCanvas, fullRoomDisplay, 13.*3., 9.*3.)  // Avalonia ScaleTransform is 'about the center of the element', need to translate to keep same upper-left
            let textWidth = 340.
        ...
        let onClick(ea:Input.PointerPressedEventArgs, curState) = 
            let pp = ea.GetCurrentPoint(popupCanvas)
            if pp.Properties.IsLeftButtonPressed || pp.Properties.IsRightButtonPressed then 
                CustomComboBoxes.DismissPopupWithResult(curState, pp.Properties.IsLeftButtonPressed)
            else
        ...
        popupCanvas.Children.Remove(tileSurroundingCanvas) |> ignore
        match r with
        | None -> return true
    ...
    let makeBorderMouseOverBehavior(b:Border, otherEnterEffect, otherLeaveEffect) =
        b.PointerEnter.Add(fun _ -> b.BorderBrush <- Brushes.Lime; otherEnterEffect())
        b.PointerLeave.Add(fun _ -> b.BorderBrush <- Brushes.Gray; otherLeaveEffect())
    ...
    if allDone then
        ...
        let handlePossibleConfirmAndDismiss(ea:Input.PointerPressedEventArgs) =
            ea.Handled <- true
            let pp = ea.GetCurrentPoint(popupCanvas)
            if pp.Properties.IsLeftButtonPressed || pp.Properties.IsRightButtonPressed then 
        ...
        popupCanvas.PointerPressed.Add(fun _ -> cleanupAndUnblock())
        let redrawWorkingCopy() =
            if workingCopyDisplay <> null then
                popupCanvas.Children.Remove(workingCopyDisplay) |> ignore
            workingCopyDisplay <- new LayoutTransformControl(Child=new Border(BorderBrush=Brushes.Gray, BorderThickness=Thickness(1.), Child=workingCopy.CurrentDisplay(false)))
            workingCopyDisplay.LayoutTransform <- new ScaleTransform(SCALE, SCALE)
            RenderOptions.SetBitmapInterpolationMode(workingCopyDisplay, Avalonia.Visuals.Media.Imaging.BitmapInterpolationMode.LowQuality)
            workingCopyDisplay.IsHitTestVisible <- true
            makeBorderMouseOverBehavior(workingCopyDisplay.Child :?> Border, (fun () -> drmbed.BorderBrush <- Brushes.Lime), (fun () -> drmbed.BorderBrush <- Brushes.Gray))
            workingCopyDisplay.PointerPressed.Add(fun ea -> handlePossibleConfirmAndDismiss(ea))
        ...
        canvasAdd(popupCanvas, explainer1, 280., tileY-430.)
        ...
        b.PointerPressed.Add(fun ea -> handlePossibleConfirmAndDismiss(ea))   
