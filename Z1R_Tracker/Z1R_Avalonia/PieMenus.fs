...
let FourWayPieMenu(cm,h,bordersDocksBehaviors:(Border*_*_)[]) = async {
    ...
    let slash1 = new Shapes.Line(StartPoint=Point(0., w), EndPoint=Point(delta, w-delta), Stroke=targetBrush, StrokeThickness=3., HorizontalAlignment=HorizontalAlignment.Center, VerticalAlignment=VerticalAlignment.Center)
    lineCanvas.Children.Add(slash1) |> ignore
    let slash2 = new Shapes.Line(StartPoint=Point(w-delta, delta), EndPoint=Point(w, 0.), Stroke=targetBrush, StrokeThickness=3., HorizontalAlignment=HorizontalAlignment.Center, VerticalAlignment=VerticalAlignment.Center)
    lineCanvas.Children.Add(slash2) |> ignore
    let backslash1 = new Shapes.Line(StartPoint=Point(0., 0.), EndPoint=Point(delta, delta), Stroke=targetBrush, StrokeThickness=3., HorizontalAlignment=HorizontalAlignment.Center, VerticalAlignment=VerticalAlignment.Center)
    lineCanvas.Children.Add(backslash1) |> ignore
    let backslash2 = new Shapes.Line(StartPoint=Point(w-delta, w-delta), EndPoint=Point(w, w), Stroke=targetBrush, StrokeThickness=3., HorizontalAlignment=HorizontalAlignment.Center, VerticalAlignment=VerticalAlignment.Center)
