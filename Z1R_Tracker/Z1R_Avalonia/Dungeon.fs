...
// door colors
let unknown = new SolidColorBrush(Color.FromRgb(40uy, 40uy, 50uy)) :> IBrush
let no = Brushes.DarkRed :> IBrush
let yes = new SolidColorBrush(Color.FromRgb(0uy,180uy,0uy)) :> IBrush
let blackedOut = new SolidColorBrush(Color.FromRgb(15uy, 15uy, 25uy))  :> IBrush
            ...
            for y = 0 to 7 do
                ...
                if i>=0 && i<=7 && j>=0 && j<=7 then
                    if grabContiguousRooms.[i,j] then
                        ...
                        let door(target:Canvas[,], x, y, source:IBrush[,], i, j) =
