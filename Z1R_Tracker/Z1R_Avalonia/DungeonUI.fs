...
let MakeLocalTrackerPanel(cm:CustomComboBoxes.CanvasManager, pos:Point, sunglasses, level) =
    ...
    let line,triangle = Graphics.makeArrow(30.*float dungeonIndex+15., 36.+float(d.Boxes.Length+1)*30., pos.X+24., pos.Y-3.+float TH, yellow)
    ...
    let unhighlight() = 
        cm.AppMainCanvas.Children.Remove(c) |> ignore
        link1.Opacity <- 1.
        link2.Opacity <- 0.
        line.Opacity <- 0.
        triangle.Opacity <- 0.
    ...
    sp.PointerEnter.Add(fun _ -> highlight())
    sp.PointerLeave.Add(fun _ -> unhighlight())
    border, unhighlight
...
let makeDungeonTabs(cm:CustomComboBoxes.CanvasManager, posY, selectDungeonTabEvent:Event<int>, TH, rightwardCanvas:Canvas, contentCanvasMouseEnterFunc, contentCanvasMouseLeaveFunc) =
    ...
    for level = 1 to 9 do
        ...
        let LD_X, LD_Y = contentCanvas.Width-localDungeonTrackerPanelWidth, blockerGridHeight - float(TH) - 3.
        ...
        do
            let PopulateLocalDungeonTrackerPanel() =
                ...
                if localDungeonTrackerPanel<> null then
                    contentCanvas.Children.Remove(localDungeonTrackerPanel) |> ignore  // remove old one
        ...
        for i = 0 to 6 do
            for j = 0 to 7 do
                ...
                let line = new Shapes.Line(StartPoint=Point(6.,-12.), EndPoint=Point(6.,28.), StrokeThickness=3., Stroke=no, Opacity=0.)
        ...
        for i = 0 to 7 do
            for j = 0 to 6 do
                ...
                let line = new Shapes.Line(StartPoint=Point(-14.,6.), EndPoint=Point(38.,6.), StrokeThickness=3., Stroke=no, Opacity=0.)
                d.Children.Add(rect) |> ignore
                d.Children.Add(line) |> ignore
                let door = new Dungeon.Door(Dungeon.DoorState.UNKNOWN, (function 
                    | Dungeon.DoorState.YES        -> rect.Stroke <- yes; rect.Fill <- yes; rect.Opacity <- 1.; line.Opacity <- 0.
        ...
        hoverCanvas.PointerEnter.Add(fun _ ->
        ...
        hoverCanvas.PointerLeave.Add(fun _ -> hoverCanvas.Children.Clear(); canvasAdd(hoverCanvas, miniBorder, 0., 0.))
        ...
        ToolTip.SetTip(bigIconsCB, "Toggle whether larger or smaller corner icons are shown on dungeon rooms")
        ...
        bigIconsBorder.PointerEnter.Add(fun _ ->
            bigIconsTemp <- true
            redrawAllRooms()
            ...
            rightwardCanvas.Height <- dungeonTabs.Height
            rightwardCanvas.Children.Add(border) |> ignore
            )
        bigIconsBorder.PointerLeave.Add(fun _ ->
            bigIconsTemp <- false
            redrawAllRooms()
            ...
            for j = 0 to 7 do
                ...
                let redraw() =
                    ...
                    if roomIsCircled.[i,j] then
                        ...
                        ellipse.StrokeDashArray <- new Collections.AvaloniaList<_>( seq[0.;12.5;8.;15.;8.;15.;] )
                ...
                let activatePopup(positionAtEntranceRoomIcons) = async {
                    popupIsActive <- true
                    let roomPos = c.TranslatePoint(Point(), cm.AppMainCanvas).Value
                    ...
                    c.Children.Remove(dashCanvas) |> ignore
                    Graphics.WarpMouseCursorTo(pos)
                    redraw()
                    popupIsActive <- false
                    }
                ...
                c.PointerWheelChanged.Add(fun _ -> 
                    if not popupIsActive then
